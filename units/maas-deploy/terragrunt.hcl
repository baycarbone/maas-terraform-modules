...
terraform {
  ...
  source = "git::https://github.com/canonical/maas-terraform-modules.git//modules/maas-deploy?ref=${try(coalesce(values.version), "")}"
}
...
inputs = merge(
  ...
  },
  {
    ...
    juju_cloud_name = try(
      coalesce(
        values.juju_cloud_name,
        dependency.juju_bootstrap.outputs.juju_cloud
      ),
      null
    )

    juju_credentials = try(
      coalesce(
        values.juju_credentials,
        dependency.juju_bootstrap.outputs.juju_credentials
      ),
      null
    )
  },
